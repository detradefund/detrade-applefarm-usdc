...
class SpotBalanceManager:
    ...
    def get_balances(self, address: str) -> Dict[str, Any]:
        ...
        """
        print("SPOT BALANCE MANAGER - Etherlink Apple XTZ")
        
        print("\nProcessing method:")
        print("  - Querying balanceOf(address) for Apple XTZ token")
        print("  - Getting live WXTZ/USDC price from GeckoTerminal")
        print("  - Skipping native XTZ balance (excluded)")
        
        # Get WXTZ/USDC price once for all conversions
        ...
        try:
            ...
            try:
                # Skip native XTZ balance check - we only want token balances
                print("\nSkipping native XTZ balance (excluded from spot tracking)")
            except Exception as e:
                print(f"Error checking native XTZ balance: {str(e)}")
            
            # Process Apple XTZ token
            for token_type, network_contracts in self.contracts.items():
                ...
                if balance > 0:
                    # Apple XTZ token - convert to WXTZ (1:1), then WXTZ to USDC using live rate
                    wxtz_amount = balance  # 1 applXTZ = 1 WXTZ
                    
                    # Convert WXTZ to USDC using live rate
                    usdc_amount = str(int(Decimal(wxtz_amount) * Decimal(wxtz_usdc_rate) * Decimal(10**6) / Decimal(10**18)))
                    
                    network_total += int(usdc_amount)
                    total_applxtz_wei += int(usdc_amount)
                        ...
                        "value": {
                            "WXTZ": {
                                ...
                                "conversion_details": {
                                    ...
                                    "fallback": False,
                                    "note": "1 applXTZ = 1 WXTZ (fixed rate)"
                                }
                            },
                            "USDC": {
                                "amount": usdc_amount,
            ...
            if total_applxtz_wei > 0:
                result["totals"] = {
                    "wei": total_applxtz_wei,
                    "formatted": f"{total_applxtz_wei/1e18:.6f}"
                }
